...
class BlogCatalogDataset(Dataset):
    def __init__(
        ...
        super(BlogCatalogDataset, self).__init__(root, transform, pre_transform)

    @property
    def raw_file_names(self):
        return ["blogcatalog.mat"]
    ...
    def process(self):
        ...
        mat_content = scipy.io.loadmat(Path(self.raw_dir) / "blogcatalog.mat")
...
if __name__ == "__main__":
    root_path = Path(__file__).parents[1] / "Data" / "BlogCatalog"
    # "flickr.mat" must be in the folder Data/Flickr/raw
    dataset = BlogCatalogDataset(root=str(root_path), compute_features=True)
    print(dataset[0])
